...
One key difference between a checkpoint and a commit is that a commit 
requires all users to have the node associated with the commit in their
DAG. In other words, all clients need to explicity agree on which document 
version corresponds to a particular name. This condition is met once the
visibility list for a named checkpoint contains all users associated with
a document.
