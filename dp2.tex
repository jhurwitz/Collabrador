...
consisting of \textbf{checkpoint objects}, keyed by the SHA-1 hash of
the object. For full implementation details, see
Section~\ref{sec:storage-dag}. A checkpoint object is a data structure
...
stored this checkpoint. We will assume that SHA-1 has no collisions,
...
directed acyclic graph (DAG) instead. (For this reason, this paper
...
checkpoint database only the portion of the graph that it knows about
while a synchronization process in the background lets users
